...
export class GeneralWaybillService {
   ...
   */
  async generatePDF(departure: Departure, shipments: any[]): Promise<string> {
    console.log(
      `🔄 [PDF] Génération du nouveau PDF pour le départ #${departure.id} avec le nouveau design`
    );
    ...
    if (departure.pdf_path && fs.existsSync(departure.pdf_path)) {
      try {
        ...
        console.warn(
          `⚠️  [PDF] Impossible de supprimer l'ancien fichier: ${error}`
        );
      }
    }
    ...
    try {
      ...
      console.warn(
        "⚠️ [PDF] Erreur lors de la récupération des settings:",
        error
      );
    }
    ...
    const activityDesc =
      "Collecte – Traitement – Acheminement et Distribution de Colis et Courriers";
    ...
    const companyInfo =
      "Siège social Mvan B.P 5633 Yaoundé - Tél : 677 891 404 / 697 06 63 50";
    ...
    doc.text("BORDEREAU D'EXPÉDITION", 50, yPos, {
      align: "center",
      width: pageWidth,
    });
    ...
    doc.text(`N° : ${waybillNumber}`, 50, yPos, {
      align: "center",
      width: pageWidth,
    });
    ...
    doc.text(departureOffice, value1ZoneX, currentY, {
      width: valueZoneWidth,
      align: "center",
    });
    // Center the underline under the value
    const underline1Width = Math.max(80, getTextWidth(departureOffice, 7));
    const underline1StartX =
      value1ZoneX + valueZoneWidth / 2 - underline1Width / 2;
    doc
      .moveTo(underline1StartX, currentY + underlineYOffset)
      .lineTo(underline1StartX + underline1Width, currentY + underlineYOffset)
      .stroke();
    ...
    doc.fontSize(7).font("Helvetica");
    // Center the value text in the zone
    doc.text(destinationOffice, value2ZoneX, currentY, {
      width: valueZoneWidth,
      align: "center",
    });
    // Center the underline under the value
    const underline2Width = Math.max(80, getTextWidth(destinationOffice, 7));
    const underline2StartX =
      value2ZoneX + valueZoneWidth / 2 - underline2Width / 2;
    doc
      .moveTo(underline2StartX, currentY + underlineYOffset)
      .lineTo(underline2StartX + underline2Width, currentY + underlineYOffset)
      .stroke();
    ...
    const vehicleValue = departure.vehicle
      ...
      : "";
    // Center the value text in the zone
    doc.text(vehicleValue, value3ZoneX, currentY, {
      width: valueZoneWidth,
      align: "center",
    });
    // Center the underline under the value
    const underline3Width = Math.max(80, getTextWidth(vehicleValue, 7));
    const underline3StartX =
      value3ZoneX + valueZoneWidth / 2 - underline3Width / 2;
    doc
      .moveTo(underline3StartX, currentY + underlineYOffset)
      .lineTo(underline3StartX + underline3Width, currentY + underlineYOffset)
      .stroke();
    ...
    doc.fontSize(7).font("Helvetica");
    // Center the value text in the zone
    doc.text(departureDate, value4ZoneX, currentY, {
      width: valueZoneWidth,
      align: "center",
    });
    // Center the underline under the value
    const underline4Width = Math.max(80, getTextWidth(departureDate, 7));
    const underline4StartX =
      value4ZoneX + valueZoneWidth / 2 - underline4Width / 2;
    doc
      .moveTo(underline4StartX, currentY + underlineYOffset)
      .lineTo(underline4StartX + underline4Width, currentY + underlineYOffset)
      .stroke();
    ...
    const driverValue = departure.driver
      ...
      : "";
    // Center the value text in the zone
    doc.text(driverValue, value5ZoneX, currentY, {
      width: valueZoneWidth,
      align: "center",
    });
    // Center the underline under the value
    const underline5Width = Math.max(80, getTextWidth(driverValue, 7));
    const underline5StartX =
      value5ZoneX + valueZoneWidth / 2 - underline5Width / 2;
    doc
      .moveTo(underline5StartX, currentY + underlineYOffset)
      .lineTo(underline5StartX + underline5Width, currentY + underlineYOffset)
      .stroke();
    ...
    const underline6StartX =
      value6ZoneX + valueZoneWidth / 2 - underline6Width / 2;
    // Underline for empty value (centered)
    doc
      .moveTo(underline6StartX, currentY + underlineYOffset)
      .lineTo(underline6StartX + underline6Width, currentY + underlineYOffset)
      .stroke();
    ...
    doc
      .rect(tableStartX, tableTop - 5, tableWidth, 18)
    ...
    doc.text("N° Bordereau", colStartX, tableTop, {
      width: colWidths.waybill - padding * 2,
      align: "center",
    });

    colStartX += colWidths.waybill;
    doc.text("Expéditeur", colStartX + padding, tableTop, {
      width: colWidths.sender - padding * 2,
      align: "center",
    });
    ...
    doc.text("Poids", colStartX + padding, tableTop, {
    ...
    for (const shipment of shipments) {
      if (yPos > 680) {
        ...
        doc
          .rect(tableStartX, newTableTop - 5, tableWidth, 18)
          .fillAndStroke("#E5E7EB", "#000000");
        ...
        doc.text("N° Bordereau", newColStartX, newTableTop, {
          width: colWidths.waybill - padding * 2,
          align: "center",
        });

        newColStartX += colWidths.waybill;
        doc.text("Expéditeur", newColStartX + padding, newTableTop, {
          width: colWidths.sender - padding * 2,
          align: "center",
        });
        ...
        doc.text("Poids", newColStartX + padding, newTableTop, {
          width: colWidths.weight - padding * 2,
          align: "center",
        });
        yPos = newTableTop + 18;
      ...
      doc.text(shipment.waybill_number || "N/A", dataColStartX, rowY, {
        width: colWidths.waybill - padding * 2,
        align: "center",
      });
      ...
      doc.text(shipment.sender_name || "N/A", dataColStartX + padding, rowY, {
        width: colWidths.sender - padding * 2,
        align: "center",
      });
      ...
      doc
        .moveTo(tableStartX, rowY + rowHeight - 3)
        .lineTo(tableStartX + tableWidth, rowY + rowHeight - 3)
        .stroke();
    ...
    doc
      .moveTo(tableStartX + tableWidth, tableTop - 5)
      .lineTo(tableStartX + tableWidth, yPos - 3)
      .stroke();
    ...
    doc
      .fontSize(8)
      .fillColor("#666666")
      .text("Nom et signature", 50, yPos + 25);
    ...
    doc
      .fontSize(8)
      .fillColor("#666666")
      .text("Nom et signature", 300, yPos + 25);
    ...
    if (departure.notes) {
      ...
      doc
        ...
        .fontSize(10)
        .font("Helvetica-Bold")
        .text("REMARQUES", 50, yPos);
      doc
        .fontSize(9)
        .font("Helvetica")
        .text(departure.notes, 50, yPos + 15, { width: 500 });
    }
