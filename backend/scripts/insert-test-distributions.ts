...
async function generateTestDistributions() {
  try {
    ...
    if (!testUser) {
      ...
      if (users.length === 0) {
        throw new Error(
          "Aucun utilisateur trouvé. Veuillez créer un utilisateur d'abord."
        );
      }
      testUser = users[0];
    ...
    if (vehicles.length < 4) {
      throw new Error(
        "Il faut au moins 4 véhicules. Exécutez: npm run seed:vehicles"
      );
    }

    if (drivers.length < 4) {
      throw new Error(
        "Il faut au moins 4 chauffeurs. Exécutez: npm run seed:drivers"
      );
    }

    console.log(
      `✅ Found ${vehicles.length} vehicles and ${drivers.length} drivers`
    );
    ...
    for (let depIndex = 0; depIndex < 4; depIndex++) {
      ...
      console.log(
        `✅ Departure ${savedDeparture.id} created: ${routes[depIndex]}`
      );
      ...
      for (let shipIndex = 0; shipIndex < 10; shipIndex++) {
        ...
        const waybillNumber = `${prefix}${shipmentNumber
          .toString()
          .padStart(4, "0")}`;
        shipmentNumber++;

        const declaredValue = parseFloat(
          (config.price * 2 + Math.random() * 50000).toFixed(2)
        );
      ...
      console.log(
        `✅ Created ${savedShipments.length} shipments for departure ${savedDeparture.id}`
      );
    ...
    const departureIds = departures.map((d: Departure) => d.id);
    ...
    for (const shipment of allShipments) {
      const weight = parseFloat(shipment.weight?.toString() || "0");
    ...
    console.log(
      `   - Éligibles répartition chauffeur (Colis ≤ 40kg): ${driverEligible}`
    );
